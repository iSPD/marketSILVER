...
# sys.path.append('/home/khkim/work/wave2vec2/inference/K-wav2vec/')

import editdistance
import numpy as np
import torch
from wav2vec.fairseq import checkpoint_utils, options, progress_bar, tasks, utils
from wav2vec.fairseq.data.data_utils import post_process
from wav2vec.fairseq.dataclass.utils import convert_namespace_to_omegaconf
from wav2vec.fairseq.logging.meters import StopwatchMeter, TimeMeter
import unicodedata
...
def add_asr_eval_argument():
    ...
    ## 1. task
    # Set Me
    parser.add_argument("--task", default='audio_multitraining', type=str, help="fairseq task")
    ...
    parser.add_argument(
        "--results-path",
        default='eval_log/myTest/multi_model_dev',
        help="log path",
    )
    ...
    parser.add_argument(
        "--experiments-dir",
        type=str,
        default='experiments/myTest/multi_model_dev.csv',
...
def cli_main():
    ...
    args = options.parse_args_and_arch(parser, inputPath)
